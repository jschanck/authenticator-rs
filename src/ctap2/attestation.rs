                        ...
                        "attStmt" => {
                            ...
                            match format {
                                AttestationFormat::None => {
                                    // In this case, attStmt should be an empty map. We can't
                                    // encode the emptyness requirement in the type hint to
                                    // map.next_value(), so we have to check that manually.
                                    let value: std::collections::BTreeMap<(), ()> =
                                        map.next_value()?;
                                    if !value.is_empty() {
...
pub mod test {
    ...
    const SAMPLE_ATTESTATION: [u8; 677] = [
        0xa3, // map(3)
        0x63, // text(3)
        0x66, 0x6D, 0x74, // "fmt"
        0x66, // text(6)
    ...
    fn parse_attestation_object() {
        ...
        let actual: AttestationObject = from_slice(&SAMPLE_ATTESTATION).unwrap();
