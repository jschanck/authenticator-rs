...
use crate::ctap2::attestation::{
    AAGuid, AttestationFormat, AttestationObject, AttestationStatement,
    AttestationStatementFidoU2F, AttestedCredentialData, AuthenticatorData, AuthenticatorDataFlags,
};
                ...
                M: MapAccess<'de>,
            {
                ...
                while let Some(key) = map.next_key()? {
                    match key {
                        ...
                        1 => {
                            if format.is_some() {
                                return Err(DesError::duplicate_field("fmt"));
                            }
                            format = Some(map.next_value()?);
                        }
                        2 => {
                            if auth_data.is_some() {
                                return Err(DesError::duplicate_field("auth_data"));
                            }
                        ...
                        3 => {
                            let format = format
                                .as_ref()
                                .ok_or_else(|| DesError::missing_field("fmt"))?;
                            if att_statement.is_some() {
                                return Err(DesError::duplicate_field("att_statement"));
                            }
                            match format {
